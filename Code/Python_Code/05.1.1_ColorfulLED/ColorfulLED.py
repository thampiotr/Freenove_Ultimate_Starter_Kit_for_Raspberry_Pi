...
def setup():
    ...
    pwmRed = GPIO.PWM(pins[0], 2000)  # set PWM Frequency to 2kHz
    pwmGreen = GPIO.PWM(pins[1], 2000)  # set PWM Frequency to 2kHz
    pwmBlue = GPIO.PWM(pins[2], 2000)  # set PWM Frequency to 2kHz
        ...
        for r in range(0, 100, 10):
            for g in range(0, 100, 10):
                for b in range(0, 100, 10):
                    setColor(r, g, b)
                    time.sleep(0.05)
                for b in reversed(range(0, 100, 10)):
                    setColor(r, g, b)
                    time.sleep(0.05)
            for g in reversed(range(0, 100, 10)):
                setColor(r, g, 0)
                time.sleep(0.05)
