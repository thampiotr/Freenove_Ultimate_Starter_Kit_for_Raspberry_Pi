    ...
    def scanKeys(self):
        ...
        for pin_c in self.colPins:
            ...
            for r in self.rowPins:  # keypress is active low so invert to high.
                self.bitMap[self.rowPins.index(r)] = self.bitWrite(self.bitMap[self.rowPins.index(r)],
                                                                   self.colPins.index(pin_c), not GPIO.input(r))
...
def loop():
    ...
    while True:
        key = keypad.getKeys()
        if key != keypad.NULL and len(keys) > 0:
